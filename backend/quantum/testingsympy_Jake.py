...
finalPortfolio = (
    []
)  # Final data set of stocks and portfolio weights to be sent to backend
...
stock_tickers = [
    ...
    "MSFT",
    "AMZN",
]  # All of the tickers we are using (imported from backend, in order of variables)
...
granularityFactor = (
    5  # granularity of the weightings, higher the more computationally intensive
)


def findPK(
    granFactor, maxPWeight, minPWeight
):  # Function to be called inside the findWeights one to adjust the PK
...
def findWeights():  # fuction that will find the weights of the assets
    PK = findPK(
        granularityFactor, maxPortfolioWeight, minPortfolioWeight
    )  # Calling function above using variables defined at the top
    weightList = []
    for i in range(
        granularityFactor
    ):  # iterating the same number of times as the granularity
    ...
    print(
        "The maximum potential weighting of this asset in our portfolio is "
        + str((sum(weightList)))
        + ". This should be as close to "
        + str(maxPortfolioWeight)
        + " as possible."
    )
...
def main():
    ...
    final_dict = extract_variable_terms(
        square_and_expand_expression(create_squared_expression(terms))
    )
